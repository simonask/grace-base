...
namespace falling {
	...
	void PointerType::deserialize_raw(byte *place, const falling::ArchiveNode &, falling::IUniverse &) const {
		ASSERT(false); // Cannot deserialize pointer type.
	}
	
	void PointerType::serialize_raw(const byte *place, falling::ArchiveNode &, falling::IUniverse &) const {
